...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout, max_len=5000):
        ...
        # 这样我们就得到了位置编码矩阵pe, pe现在还只是一个二维矩阵，要想和embedding的输出（一个三维张量）相加，(batch,seq_len,d_word_vec)
        # 就必须拓展一个维度，所以这里使用unsqueeze拓展维度. 增加了batch的这个？编码与batch无关，不同batch相同位置的元素对应的位置编码信息相同。
        pe = pe.unsqueeze(0)
    ...
    def forward(self, x):
        ...
        # 在相加之前我们对pe做一些适配工作， 将这个三维张量的第二维也就是句子最大长度的那一维将切片到与输入的x的第二维相同即x.size(1)，就是seq_len了吧。句子的长度！
...
pe_result = pe(x) #x是上文embedding之后的结果，pe_result就是加入了位置编码信息的嵌入结果拉
...
# 因为总共有20维之多, 我们这里只查看4，5，6，7维的值. 一个单词被embedding成20维的向量。
...
def subsequent_mask(size):
    ...
    #解码器的输出却不是一次就能产生最终结果的，而是次次通过上一次结果综合得出的，因此，未来的信息可能被提前利用。所以，我们会进行遮掩。
    # 在函数中, 首先定义掩码张量的形状
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        # 这个方法的作用就是能够让转置后的张量应用view方法，否则将无法直接使用，（使用view方法进行维度重塑）
        # 所以，下一步就是使用view重塑形状，变成和输入形状相同.
        x = x.transpose(1, 2).contiguous().view(batch_size, -1, self.head * self.d_k)
        # x是上面定义的attention函数最后返回的query注意力表示（就是那个公式拉！）

        # 最后使用线性层列表中的最后一个线性层对输入进行线性变换得到最终的多头注意力结构的输出. （-1表示这里使用的线性层是__init__中克隆的四个线性层中的最后一个）
...
class SublayerConnection(nn.Module):
    def __init__(self, size, dropout=0.1):
        ...
        # 实例化了规范化对象self.norm
        self.norm = LayerNorm(size)  #size是词嵌入维度的大小
...
# 使用lambda获得一个函数类型的子层
# 假设输入的Q，K，V仍然相等，query = value = key = pe_result=x
sublayer = lambda x: self_attn(x, x, x, mask)


sc = SublayerConnection(size, dropout)
sc_result = sc(x, sublayer) #残差连接，最开始的 + 变换后的
# print(sc_result)
...
# 使用Encoder类来实现编码器，编码器是由N个编码器层组成的！
...
#第一个编码器的输入是embedding的结果，第二个编码器的输入就是第一个编码器的输出啦，往后依次类推。
...
def make_model(source_vocab, target_vocab, N=6,
    ...
    model = EncoderDecoder(
        ...
        Decoder(DecoderLayer(d_model, c(attn), c(attn),
                             c(ff), dropout), N),
        nn.Sequential(Embeddings(d_model, source_vocab), c(position)), #nn.Sequential()用来构造顺序模块（模型的组合！）
